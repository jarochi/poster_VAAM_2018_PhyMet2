...
Our rudimentary knowledge about them results from difficulties with their isolation and culturing in laboratory conditions, which are necessary to describe their phenotype. 
% Innovations in DNA sequencing 
NGS technologies allowed for rapid development of metagenomics. DNA sequencing of environmental samples resulted in identifying a plethora of new uncultivated methanogens. \\
...
In order to train MethanoGram, we used n-grams, i.e. subsequences of the length n that were extracted from 16S rRNA and mcrA. We chose only those species that have known 16S rRNA, mcrA and all important culturing conditions. 

\begin{figure}
\includegraphics[width=0.77\columnwidth]{suplem2.eps}
\end{figure}
...
ggplot(p, aes(x = type, y = error, fill = type, label = nice_label)) +
  ...
  theme(legend.position = "bottom",
        strip.background = element_rect(fill = "gold3"))
